...
**In v5**, your project had separate folders for each app: `apps/api`, `apps/admin`, `apps/website`, and `apps/core`. Each folder contained its own code, configuration, and dependencies. While this separation made sense in theory, in practice it meant a lot of jumping between directories to understand how things worked.

**In v6**, we've eliminated all those app folders. Your implementation code lives in just two places:

- `webiny.config.tsx` - declarative configuration for your entire project
- `extensions/` - all your custom code and extensions

That's it. No more hunting through `apps/api` to find backend logic, then switching to `apps/admin` for frontend customizations. Everything is in the `extensions/` folder, organized by feature rather than by technical layer.

As a bonus, v6 also simplifies the infrastructure around your project:

- No monorepo by default
- Single root `package.json`
- Just one `tsconfig.json` (goodbye `tsconfig.build.json`)
