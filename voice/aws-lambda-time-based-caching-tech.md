When developing serverless applications (applications that will be running on serverless services, like AWS Lambda), one might ask themselves: is caching still relevant since we don't have to think about cloud infrastructure and about scaling?

Well, as Yan Cui put it in his All you need to know about caching for serverless applications article, in short, yes, caching is still VERY relevant.
...
Well, if you were familiar with how AWS Lambda works, you would know that the service can spawn multiple instances of your function, each operating in its own isolated environment (container). How many instances are spawned, essentially, depends on the amount of work there is to be done. The more work needs to be done, the more instances will be spawned. And only if an instance hasn't been active for some time (10-15minutes), the instance is terminated.

The following diagram shows how this might look like when our AWS Lambda function is invoked by concurrent HTTP requests received by an Amazon API Gateway:
...
Based on the flow diagram we've shown at the beginning of this article, depending on the cached licenses, we could even end up in a situation where some AWS Lambda function instances throw an error at the start of the function invocation, while others allow for the rest of the code to be executed.
